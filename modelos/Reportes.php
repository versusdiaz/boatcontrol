...
class Reportes{
    ...
    public static function dataCentro($idcentro){
        $sql = "SELECT T1.idods_mtto, T1.codigo, T2.nombre, T1.com_falla, T1.fecha, T1.horas FROM ods_mtto AS T1 LEFT JOIN centro AS T2 ON T1.idcentro = T2.idcentro WHERE T1.idcentro = $idcentro";
        return Consulta($sql);
    }
